( function () {
   ...
   */
	...
	p( 'Math.asinh', function ( a ) {
		return a
			...
			: function ( b ) {
					...
					const c = Math.log( Math.abs( b ) + Math.sqrt( b * b + 1 ) );
					return 0 > b ? -c : c;
			  };
	} );
	...
	p( 'Math.cbrt', function ( a ) {
		return a
			...
			: function ( b ) {
					...
					b = Number( b );
					const c = Math.pow( Math.abs( b ), 1 / 3 );
					return 0 > b ? -c : c;
			  };
	} );
	...
	p( 'Object.fromEntries', function ( a ) {
		return a
			...
			: function ( b ) {
					...
					for ( let d = b.next(); ! d.done; d = b.next() ) {
						d = d.value;
						if ( Object( d ) !== d ) {
							throw new TypeError(
		...
		return a
			...
			: function ( b ) {
					for (
						...
						c[ d[ e ] ] = Object.getOwnPropertyDescriptor(
							b,
							d[ e ]
						);
					}
	...
	p( 'Reflect.apply', function ( a ) {
		...
		const b = Function.prototype.apply;
	...
	p( 'Reflect.set', function ( a ) {
		return a
			...
			: function ( b, c, d, e ) {
					...
					return h
						? h.set
							? ( h.set.call( 3 < arguments.length ? e : b, d ),
							  ! 0 )
	...
	function qb( a, b ) {
		...
		b = Q( a, b );
		a.g.has( b ) &&
			( ( a.i = null ), ( a.h -= a.g.get( b ).length ), a.g.delete( b ) );
	}
	function rb( a, b ) {
		P( a );
		b = Q( a, b );
	...
	N.prototype.toString = function () {
		...
		for (
			...
			for ( let h = 0; h < d.length; h++ ) {
				let f = e;
				'' !== d[ h ] &&
					( f += '=' + encodeURIComponent( String( d[ h ] ) ) );
				a.push( f );
			}
		}
	...
	function nb( a, b ) {
		b &&
			...
			a.g.forEach( function ( c, d ) {
				...
				if (
					d != e &&
					( qb( this, d ), qb( this, e ), 0 < c.length )
				) {
					this.i = null;
					d = this.g;
					const h = d.set;
	...
	function Bb() {
		...
		'undefined' === typeof a &&
			...
			-1 == $a().indexOf( 'Presto' ) &&
			( a = function () {
				let e = vb( document, 'IFRAME' );
				e.style.display = 'none';
				document.documentElement.appendChild( e );
				const h = e.contentWindow;
				e = h.document;
				e.open();
				e.close();
				const f = 'callImmediate' + Math.random(),
					g =
	...
	function nc( a ) {
		...
		0 < k &&
			( ( g = window.setTimeout( function () {
				kc( e, ! 0 );
				const m = new lc( 1, 'Timeout reached for loading script ' + d );
				ac( f );
				W( f, ! 1, m );
			}, k ) ),
			( h.M = g ) );
		...
		e.onerror = function () {
			kc( e, ! 0, g );
			const m = new lc( 0, 'Error while loading script ' + d );
			ac( f );
			W( f, ! 1, m );
		};
		h = b.attributes || {};
   ...
   */
	...
	function Lc( a, b, c ) {
		...
		if ( d ) {
			...
			const e = { module: a, version: b || '', file: d.compressed };
	...
	if ( sc ) {
		...
		let Nc = document.getElementsByTagName( 'script' ),
		...
		sc = Qc = Qc.match( /^www\.gstatic\.cn/ )
			? 'gstatic.cn'
			: 'gstatic.com';
		const Rc = new N( Pc.i.toString() ),
			Sc = Rc.get( 'callback' );
		if ( 'string' === typeof Sc ) {
			const Tc = Bc( Sc );
			pc().then( Tc );
		}
		const Uc = Rc.get( 'autoload' );
		if ( 'string' === typeof Uc ) {
			try {
				if ( '' !== Uc )
					{for (
